...
4. Random nonces are generated for each chunk and prepended to the encrypted chunk (chunk size defaults to 200kB or less for the last chunk).
...
`<header>.<header>.<header>..<nonce><chunk><nonce><chunk>`
...
Each chunk is prepended with a 24-byte nonce and in it's encrypted form is 16-bytes longer than decrypted.
